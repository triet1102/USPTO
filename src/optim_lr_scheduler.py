...
class WarmupLRScheduler(torch.optim.lr_scheduler._LRScheduler):
    def __init__(
        ...
        print("Init LRScheduler")
        print(f"num_warmups: {num_warmups}, num_decreases: {num_decreases}")
        self.min_lr = 1e-6
    ...
    def step(self, val_loss=None):
        # print(f"\nwarmup_step: {self.update_warmup_step}; decrease_step: {self.update_decrease_step}; learning rate: {self.lr:.10f}")
        if self.update_warmup_step <= self.num_warmups:
            ...
            lr = max(self.min_lr, self.min_lr + self.decrease_rate * \
                (self.num_decreases - self.update_decrease_step))
            self.set_lr(self.optimizer, lr)
            self.lr = lr
            self.update_decrease_step += 1
